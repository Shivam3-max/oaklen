...
function shell(body: string) {
  return `<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#2B2117;background:#F7F4EF;padding:32px">
    ...
    <div style="height:1px;background:rgba(18,17,17,0.13);margin:20px 0"></div>
    ${body}
    <div style="height:1px;background:rgba(18,17,17,0.13);margin:24px 0"></div>
...
export async function sendOrderConfirmation(order: Order) {
  ...
  const body = `
    ...
    <table style="width:100%;font-size:14px;border-collapse:collapse">${itemRows(order)}
      <tr><td style="padding-top:12px;border-top:1px solid rgba(18,17,17,0.13)">Total</td><td style="padding-top:12px;border-top:1px solid rgba(18,17,17,0.13);text-align:right">${formatINR(order.subtotal)}</td></tr>
    </table>
